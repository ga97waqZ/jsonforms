...
test('update via event', t => {
    ...
    change(input);
    setTimeout(() => t.is(getData(store.getState()).foo, '1961-04-12T20:15'), 100);
});
...
test('update via action', t => {
    ...
    const input = findRenderedDOMElementWithTag(tree, 'input') as HTMLInputElement;
    store.dispatch(update('foo', () => '1961-04-12T20:15'));
    setTimeout(() => t.is(input.value, '1961-04-12T20:15'), 100);
});
...
test.failing('update with null value', t => {
    ...
    t.is(getData(store.getState()).foo, '1970-01-01T13:37');
});
